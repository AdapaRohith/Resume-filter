 ...
 * ============================================
 * 
 * FOR USE ON: https://resume-filter.avlokai.com (and other protected projects)
 * 
 * This script handles:
...
// ============================================
const PROJECT_ID = 'resume-filter';
...
const CUSTOM_DOMAINS = {
    ...
    resume_filter: 'resume-filter.avlokai.com'
};
...
function getLoginUrl() {
    ...
    // 1. Local development
    // Assuming resume-filter runs on localhost (e.g., port 3000)
    // And login runs on localhost:3001
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
        return 'http://localhost:3001/';
    }

    // 2. Custom domain (resume-filter.avlokai.com) → go to login.avlokai.com
    if (hostname === CUSTOM_DOMAINS.resume_filter) {
        return `https://${CUSTOM_DOMAINS.login}/`;
    }

    // 3. Vercel deployment → redirect to production custom domain
    if (hostname.endsWith('.vercel.app')) {
        console.log('[resume-filter] Vercel deployment detected, using production login URL');
        return `https://${CUSTOM_DOMAINS.login}/`;
    }
...
console.log('[resume-filter] Login URL:', AUTH_GUARD_CONFIG.LOGIN_PAGE_URL);
...
function showMainApp() {
    ...
    document.title = 'AvlokAi Resume Filter';
}
